...
class Imagen
{
	...
	public function changeDimensions( $iNewWidth, $iNewHeight, $maxWidth=10000, $maxHeight=10000 )
	{
		//validate parameters
		if( ( $iNewWidth = abs(intval( $iNewWidth, 10 )) ) && $iNewHeight = abs(intval( $iNewHeight, 10 )) )
		{
			if( $iNewWidth <= $maxWidth  && $iNewHeight <= $maxHeight )
			{
				//create new and original canvas
				if( $originalCanvas = $this->createCanvas() )
				{
					$newCanvas = imagecreatetruecolor( $iNewWidth, $iNewHeight );

					//preserve transparency
					if( $this->formato === "png" ) 
					{
						$originalCanvas = $this->preserveTransparency( $originalCanvas );
						$newCanvas = $this->preserveTransparency( $newCanvas );
					}
				
					//copy/merge both canvas
					imagecopyresampled( $newCanvas, $originalCanvas, 0,0,0,0, $iNewWidth, $iNewHeight, $this->ancho, $this->alto );
					
					//set new width and height properties
					$this->setAncho( $iNewWidth );
					$this->setAlto( $iNewHeight );
					$this->setError( '' );
					
					//remove original canvas and return new one
			...
			{
				$this->setError( 'Las dimensiones introducidas son demasiado grandes. Máximo '.$maxWidth.' x '.$maxHeight );
			}
		}
		else
		{
			$this->setError( 'Los datos introducidos no son enteros' );
		}
	}		
	
	public function cropImage( $iLeft, $iRight, $iTop, $iBottom )
	{
		//validate parameters
		if( is_numeric($iLeft) && $iLeft>=0 && is_numeric($iRight) && $iRight>=0 && is_numeric($iTop) && $iTop>=0 && is_numeric($iBottom) && $iBottom>=0 )
		{			
			if( ($iLeft + $iRight) < $this->ancho && ($iTop + $iBottom) < $this->alto )
			{
				//create new and original canvas
				if( $originalCanvas = $this->createCanvas() )
				{
					
					$newCanvas=imagecreatetruecolor( $this->ancho - $iLeft - $iRight, $this->alto - $iTop - $iBottom );
					...
					imagecopyresampled($newCanvas, $originalCanvas,0,0, $iLeft, $iTop, $this->ancho - $iLeft - $iRight, $this->alto - $iBottom - $iTop, $this->ancho - $iLeft - $iRight, $this->alto - $iBottom - $iTop );

					//set new width and height properties
					$this->setAlto(floor( $this->alto - $iTop - $iBottom ));
					$this->setAncho(floor( $this->ancho - $iRight - $iLeft ));
					$this->setError( '' );
			...
			{
				$this->setError( 'Los valores de recorte no son correctos' );
			}
		}
		else
		{
			$this->setError( 'Los valores de recorte deben ser numéricos' );
		}
	}		
	...
	{
		//validate parameters
		if( is_numeric($rotation) && $rotation<=360 && $rotation>=0 )
		{			
			//create new and original canvas
			if( $newCanvas = $this->createCanvas() )
			{
				...
				$newCanvas = imagerotate( $newCanvas, $rotation, 0 );
				$this->setAlto(imagesy($newCanvas));
				$this->setAncho(imagesx($newCanvas));	
		...
		{
			$this->setError( 'El ángulo debe ser un número entre 0 y 360' );
		}
	}	
	...
	public function overlapEffect( $ImName )
	{
		...
		if( $newCanvas = $this->createCanvas() )
		{
			...
			$newCanvas = $this->mergeImage( $newCanvas, $this->ancho, $this->alto, $overlapImage );			
...
/*	public function brightnessImage( $brightness )
	{
		//validate parameters
		if( is_numeric($brightness) && $brightness<256 && $brightness>-256 )
		{			
			//create new and original canvas
			if( $newCanvas = $this->createCanvas() )
			{
				...
				imagefilter( $newCanvas, IMG_FILTER_BRIGHTNESS, $brightness );
		...
		{
			$this->setError( 'El brillo debe ser un número entre -255 y 255' );
		}
	}

	public function contrastImage( $contrast )
	{
		//validate parameters
		if( is_numeric($contrast) && $contrast<=1000 && $contrast>=-1000 )
		{			
			//create new and original canvas
			if( $newCanvas = $this->createCanvas() )
			{
				...
				imagefilter( $newCanvas, IMG_FILTER_CONTRAST, $contrast );
		...
		{
			$this->setError( 'El contraste debe ser un número entre -1000 y 1000' );
		}
	}		
*/
	...
	{			
		...
		$canvas = imagecreatefromjpeg($this->ruta);
		elseif($this->formato == 'gif')
		$canvas = imagecreatefromgif($this->ruta);
		elseif($this->formato == 'png') 
		$canvas = imagecreatefrompng($this->ruta);
		elseif($this->formato == 'wbmp')
		$canvas = imagecreatefromwbmp($this->ruta);
		return $canvas;
	...
	{			
		//Image is created in 'ruta' directory
		if( $this->formato == 'jpeg' || $this->formato == 'jpg' )
		imagejpeg( $canvas, $this->ruta, $calidad );
		
		elseif( $this->formato == 'gif' ) 
		imagegif( $canvas, $this->ruta, $calidad );
		
		elseif( $this->formato == 'png' ) 
		{
			$this->preserveTransparency();
			imagepng( $canvas, $this->ruta, $calidad );
		}
		elseif( $this->formato == 'wbmp' )
		imagewbmp( $canvas, $this->ruta, $calidad );
	}
	...
	private function mergeImage( $lienzo, $ancho, $alto, $rutaImagen ) 
	{
